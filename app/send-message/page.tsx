...
import { Send, AlertCircle, CheckCircle, User } from "lucide-react"
...
export default function SendMessagePage() {
  ...
  useEffect(() => {
    if (!currentUser || !currentToken) {
      router.push(`/buyer/login?redirect=/send-message${vendorId ? `?vendorId=${vendorId}` : ""}`)
      return
    }
  ...
  const loadVendorData = async () => {
    try {
      ...
      console.error("[v0] Failed to load vendor:", err)
      setError("Failed to load vendor information")
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send message via user-messages API
      const res = await fetch(`${API_URL}/user-messages/send`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          receiverId: vendorData.userId || vendorData._id,
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          router.push(user ? "/dashboard" : "/buyer/dashboard")
        }, 2000)
      } else {
        const data = await res.json()
          ...
          <div className="max-w-md mx-auto px-4">
            <Card className="text-center">
              <CardContent className="p-8">
                ...
                <p className="text-muted-foreground mb-6">You need to sign in to send messages.</p>
            ...
            <Card className="mb-6 border-primary/20 bg-primary/5">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Send a message to this vendor</p>
                </div>
              </CardContent>
            </Card>
          )}
